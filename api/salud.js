...
async function cuentasDeHoy(req) {
  ...
  const deHoy = sesiones.filter((x) => x.empezo && new Date(x.empezo).toISOString().slice(0, 10) === hoy && x.cuenta);
  ...
  for (const p of lista) {
    ...
    if (libre) {
      ...
      fuera.push({ ...p, cuenta: libre.cuenta, porque: 'empezo a las ' + new Date(libre.empezo).toISOString().slice(11, 16), firme: false });
    } else {
  ...
  return {
    cuentas: todas,
    directosDeHoy: deHoy.map((x) => ({ cuenta: x.cuenta, empezo: x.empezo })),
